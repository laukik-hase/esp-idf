...
#define SHA256_DIGEST_SZ        256
#define PBKDF2_HMAC_SHA256_ITER 2048

esp_err_t esp_hmac_derive_pbkdf2_key(hmac_key_id_t key_id, const uint8_t *salt, size_t salt_len, size_t key_len, uint8_t *out_key)
{
    if (!out_key || !salt || key_len == 0 || salt_len == 0) {
    ...
    while (remaining > 0) {
        ...
        for (size_t i = 1; i < PBKDF2_HMAC_SHA256_ITER; i++) {
            err = esp_hmac_calculate(key_id, U, SHA256_DIGEST_SZ, U);
            if (err != ESP_OK) {
                goto cleanup;
            }

            for (size_t j = 0; j < SHA256_DIGEST_SZ; j++) {
                T[j] ^= U[j];
            }
        }
