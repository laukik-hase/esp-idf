...
static int export_ecdsa_signature(mbedtls_mpi *r, mbedtls_mpi *s, size_t key_len, esp_tee_sec_storage_ecdsa_sign_t *out_sign)
{
    if (!r || !s || !out_sign) {
        return -1;
    }

    if (mbedtls_mpi_write_binary(r, out_sign->sign_r, key_len) != 0 ||
            mbedtls_mpi_write_binary(s, out_sign->sign_s, key_len) != 0) {
...
esp_err_t esp_tee_sec_storage_ecdsa_sign(const esp_tee_sec_storage_key_cfg_t *cfg, const uint8_t *hash, size_t hlen, esp_tee_sec_storage_ecdsa_sign_t *out_sign)
{
    ...
    ret = export_ecdsa_signature(&r, &s, key_len, out_sign);
    if (ret != 0) {
        err = ESP_FAIL;
        goto exit;
    }
                                                ...
                                                esp_tee_sec_storage_ecdsa_pubkey_t *out_pubkey)
{
    ...
    err = esp_hmac_derive_pbkdf2_key(key_id, ctx->salt, ctx->salt_len, key_len, derived_key);
    if (err != ESP_OK) {
        goto exit;
    }
    ...
    if (mbedtls_mpi_write_binary(&keypair.MBEDTLS_PRIVATE(Q).MBEDTLS_PRIVATE(X), out_pubkey->pub_x, key_len) != 0 ||
            mbedtls_mpi_write_binary(&keypair.MBEDTLS_PRIVATE(Q).MBEDTLS_PRIVATE(Y), out_pubkey->pub_y, key_len) != 0) {
        err = ESP_FAIL;
        memset(out_pubkey, 0x00, sizeof(esp_tee_sec_storage_ecdsa_pubkey_t));
        goto exit;
    }
